...
- **Settings**: 9 configuration options (executablePath, configPath, profile, engines.*, severityThreshold, trace.server)
- **Assets**: CHANGELOG and LICENSE are symlinks from root (do not duplicate). `images/icon.png` is a real
  copy of `assets/vscode_icon.png` (a symlink escaping the extension root breaks packaging tools)
...
- **Version bump**: `mise run bump:patch|minor|major|pre|stage|stable` (updates 20 files). `bump:stage`
  advances one pre-release step (alpha→beta→rc→stable); `bump:stable` drops the suffix outright
